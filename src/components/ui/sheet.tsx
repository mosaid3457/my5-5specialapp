...
const SheetContent = React.forwardRef<React.ElementRef<typeof SheetPrimitive.Content>, SheetContentProps>(
  ({ side = "right", className, children, onSwipeClose, ...props }, ref) => {
    ...
    const handleTouchMove = useCallback((e: React.TouchEvent) => {
      ...
      if (side === "bottom" && diffY > 0 && Math.abs(diffY) > Math.abs(diffX)) {
        setDragOffset(Math.min(diffY, 200));
      } else if (side === "top" && diffY < 0 && Math.abs(diffY) > Math.abs(diffX)) {
        setDragOffset(Math.max(diffY, -200));
      } else if (side === "right" && diffX > 0 && Math.abs(diffX) > Math.abs(diffY)) {
        setDragOffset(Math.min(diffX, 200));
      } else if (side === "left" && diffX < 0 && Math.abs(diffX) > Math.abs(diffY)) {
        setDragOffset(Math.max(diffX, -200));
      }
    }, [side]);

    const handleTouchEnd = useCallback(() => {
      if (Math.abs(dragOffset) > threshold && onSwipeClose) {
        onSwipeClose();
      }
      setDragOffset(0);
      touchStart.current = null;
    }, [dragOffset, onSwipeClose, threshold]);
